...
describe('<%= humanizedSingularName %> CRUD tests', function() {
	beforeEach(function(done) {
		...
		user.save(function() {
			<%= camelizedSingularName %> = {
				...
				<%=prop.camelizedSingularName%>: '<%= humanizedSingularName %> Name',
				<% }); %>
			};
					...
					.end(function(<%= camelizedSingularName %>SaveErr, <%= camelizedSingularName %>SaveRes) {
						...
						agent.get('/<%= slugifiedPluralName %>')
							.end(function(<%= camelizedPluralName %>GetErr, <%= camelizedPluralName %>GetRes) {
								...
								<% properties.forEach(function(prop){ %>
								(<%= camelizedPluralName %>[0].<%=prop.camelizedSingularName%>).should.match('<%= humanizedSingularName %> Name');
								<% }); %>
	...
	it('should not be able to save <%= humanizedSingularName %> instance if no property name is provided', function(done) {
		// Invalidate property value field
		<% properties.forEach(function(prop){ %>
		<%= camelizedSingularName %>.<%=prop.camelizedSingularName%> = '';
		<% }); %>
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/<%= slugifiedPluralName %>')
					...
					.end(function(<%= camelizedSingularName %>SaveErr, <%= camelizedSingularName %>SaveRes) {
						...
						(<%= camelizedSingularName %>SaveRes.body.message).should.match('Please fill \'<%=properties[0].humanizedSingularAllLowerCaseName%>\'');
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/<%= slugifiedPluralName %>')
					...
					.end(function(<%= camelizedSingularName %>SaveErr, <%= camelizedSingularName %>SaveRes) {
						...
						// Update <%= humanizedSingularName %> property name
						<% properties.forEach(function(prop){ %>
						<%= camelizedSingularName %>.<%=prop.camelizedSingularName%> = 'WHY YOU GOTTA BE SO MEAN?';
						<% }); %>
						...
						agent.put('/<%= slugifiedPluralName %>/' + <%= camelizedSingularName %>SaveRes.body._id)
							...
							.end(function(<%= camelizedSingularName %>UpdateErr, <%= camelizedSingularName %>UpdateRes) {
								...
								<% properties.forEach(function(prop){ %>
								(<%= camelizedSingularName %>UpdateRes.body.<%=prop.camelizedSingularName%>).should.match('WHY YOU GOTTA BE SO MEAN?');
								<% }); %>
								// Call the assertion callback
								done();
							});
		...
		<%= camelizedSingularName %>Obj.save(function() {
			request(app).get('/<%= slugifiedPluralName %>/' + <%= camelizedSingularName %>Obj._id)
				.end(function(req, res) {
					// Set assertion
					<% properties.forEach(function(prop){ %>
					res.body.should.be.an.Object.with.property('<%=prop.camelizedSingularName%>', <%= camelizedSingularName %>.<%=prop.camelizedSingularName%>);
					<% }); %>
					// Call the assertion callback
					done();
				});
